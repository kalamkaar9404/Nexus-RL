...
MODEL_PATH = 'modelsave/lstmHA_net_best.pkl'
# Use GPU
    ...
    for episode in range(1, MAX_EPISODE):
        ...
        if episode % TEST_EPI == 0:
            ...
            if ave_reward>max_train_reward:
                torch.save(agent.network, 'modelsave/lstmHA_net_best.pkl')
                print("Saving best reward at episode ", episode)
                max_train_reward = ave_reward
            statistic.append(ave_reward)

        if episode % SAVE_FREQ == 0:
            torch.save(agent.network, 'modelsave/lstmHA_net_'+str(episode)+'.pkl')
...
def test():
    ...
    for j in range(MAX_STEP):
        ...
        if done:
            ...
            result.to_csv('result_lstmHA.csv',index=False)
            break
    factor_list = pd.DataFrame(factor_list)
    factor_list.to_csv('factor_lstmHA.csv',index=False)
